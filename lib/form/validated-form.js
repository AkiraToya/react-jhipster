...
function ValidatedForm(_a) {
    ...
    return (react_1.default.createElement(reactstrap_1.Form, Object.assign({ onSubmit: handleSubmit(onSubmit) }, rest), react_1.default.Children.map(children, (child) => {
        if (child.type === reactstrap_1.Button || child.type === "button")
            return child;
...
const processChild = (_a, { register, setValue, errors, touchedFields, dirtyFields }, element) => {
    var { defaultValues, children, onSubmit, mode } = _a, rest = (0, tslib_1.__rest)(_a, ["defaultValues", "children", "onSubmit", "mode"]);
    if (element.type === "button" || element.type === reactstrap_1.Button)
        return element;
    if (!element.props)
        return element;
    ...
    if (!Array.isArray(element.props.children)) {
        const newElement = processOneChild(Object.assign({ defaultValues, children, onSubmit, mode }, rest), { register, setValue, errors, touchedFields, dirtyFields }, element.props.children);
        if (element.props.children === newElement)
            return element;
        return newElement;
    }
...
const processOneChild = (_a, { register, setValue, errors, touchedFields, dirtyFields }, child) => {
    ...
    return processChild(Object.assign({ defaultValues, children, onSubmit, mode }, rest), { register, setValue, errors, touchedFields, dirtyFields }, child);
    ;
};
