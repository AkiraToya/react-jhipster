...
exports.isEmail = exports.ValidatedBlobField = exports.ValidatedField = exports.ValidatedInputAutoComplete = exports.ValidatedInput = exports.ValidatedForm = void 0;
...
function ValidatedForm(_a) {
    ...
    const { handleSubmit, register, reset, setValue, control, formState: { errors, touchedFields, dirtyFields }, } = (0, react_hook_form_1.useForm)({ mode: mode || 'onTouched', defaultValues });
    (0, react_2.useEffect)(() => {
        reset(defaultValues);
    }, [reset, defaultValues]);
    return (react_1.default.createElement(reactstrap_1.Form, Object.assign({ onSubmit: handleSubmit(onSubmit) }, rest), react_1.default.Children.map(children, (child) => {
        return processOneChild(Object.assign({ defaultValues, children, onSubmit, mode }, rest), { register, setValue, errors, touchedFields, dirtyFields, control }, child);
    })));
}
exports.ValidatedForm = ValidatedForm;
const processOneChild = (_a, { register, setValue, errors, touchedFields, dirtyFields, control }, child) => {
    var _b;
    var { defaultValues, children, onSubmit, mode } = _a, rest = (0, tslib_1.__rest)(_a, ["defaultValues", "children", "onSubmit", "mode"]);
    if (!child || !child.props || child.props === null)
        return child;
    const type = child === null || child === void 0 ? void 0 : child.type;
    if (type === "button" || type === reactstrap_1.Button)
        return child;
    const isValidated = type && ((_b = child === null || child === void 0 ? void 0 : child.props) === null || _b === void 0 ? void 0 : _b.name) && ['ValidatedField', 'ValidatedInput', 'ValidatedBlobField', 'ValidatedInputAutoComplete', 'ValidatedCustomControlledField'].includes(type.displayName);
    if (isValidated) {
    ...
    child = react_1.default.cloneElement(child, {
        children: react_1.default.Children.map(child.props.children, (nextChild) => {
            return processOneChild(Object.assign({ defaultValues, children: child.props.children, onSubmit, mode }, rest), { register, setValue, errors, touchedFields, dirtyFields, control }, nextChild);
        })
    });
    return child;
};
...
function ValidatedInputAutoComplete(_a) {
    var { name, id = name, register, error, isTouched, isDirty, validate, children, className, onChange, onBlur, options, multiple, defaultValue, customOnChange, control, row, col, check, disabled, hidden, tag, label, labelClass, labelHidden } = _a, attributes = (0, tslib_1.__rest)(_a, ["name", "id", "register", "error", "isTouched", "isDirty", "validate", "children", "className", "onChange", "onBlur", "options", "multiple", "defaultValue", "customOnChange", "control", "row", "col", "check", "disabled", "hidden", "tag", "label", "labelClass", "labelHidden"]);
    if (!register) {
        return (react_1.default.createElement(react_select_1.default, Object.assign({ name: name, id: id, className: className, onChange: customOnChange, onBlur: onBlur, options: options }, multiple ? { isMulti: true } : { isMulti: false }, defaultValue ? { defaultValue: JSON.parse(defaultValue.toString()) } : {}, attributes)));
    }
    className = className || '';
    className = isTouched ? `${className} is-touched` : className;
    className = isDirty ? `${className} is-dirty` : className;
    ...
    const input = react_1.default.createElement(react_1.default.Fragment, null,
        react_1.default.createElement(react_hook_form_1.Controller, { name: registeredName, control: control, render: ({ field }) => react_1.default.createElement(react_select_1.default, Object.assign({ id: id, className: className, placeholder: attributes.label, options: options, onChange: (e) => {
                    if (multiple) {
                ...
                } }, multiple ? { isMulti: true } : { isMulti: false }, attributes, defaultValue ? { defaultValue: JSON.parse(defaultValue.toString()) } : {})) }),
        error && react_1.default.createElement(reactstrap_1.FormFeedback, null, error.message));
...
exports.ValidatedInputAutoComplete = ValidatedInputAutoComplete;
